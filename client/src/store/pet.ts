...
export const usePetStore = defineStore('pet', {
  ...
  actions: {
     ...
     * @returns
     */
    async updatePet(petId: string, petData: Partial<Pet>): Promise<ApiResult> {
      const userStore = useUserStore();
      const headers = {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${userStore.token}`,
  ...
  getters: {
    // The current user's own pets
    getOwnerPets(state): Pet[] {
      const userStore = useUserStore();
      return state.pets.filter((pet) => pet.owner?.id === userStore.id);
    },
    // Pets shared with the current user (where they are a carer, not the owner)
    getCarerPets(state): Pet[] {
      const userStore = useUserStore();
      return state.pets.filter((pet) => pet.owner?.id !== userStore.id);
    },
    // Gets a pet by id from the state
    getPetById:
      (state) =>
      (id: string): Pet | undefined => {
        return state.pets.find((pet) => pet.id === id);
      },
    // Checks if the current user is the owner of the pet
    isOwner:
      (state) =>
