...
const isNeedActive = computed(() => needProp.value.isActive !== false);
...
const toggleNeedActive = async (needId: string | undefined) => {
  ...
  const wasActive = isNeedActive.value;
...
@media (hover: hover) {
  ...
  .need-icon-button.danger:hover {
    border-color: var(--color-danger-border);
    background: var(--color-danger-soft);
  }
}

.need-icon-button:active {
  box-shadow: var(--shadow-sm);
